...
(Special promotion texts can be requested directly towards an artwork, without using classification)
[source, cypher]
----
MATCH (n:Activity {Number:'SA3012 Rainbow'})--(a:Article)--(ms:Master_set)--(mt:Master)--(tt:onpack)--(a2:Article)
RETURN a,ms,mt,tt,a2
----
//graph_result
'''
